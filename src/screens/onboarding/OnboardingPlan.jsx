...
const INTENSITIES = [
  { id: 'gentle',   label: 'Gentle',   desc: 'Easy stretches, low effort' },
...
export function OnboardingPlan({ onSave, onBack }) {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: BG, overflowY: 'auto', padding: '16px 22px 28px' }}>
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20, flexShrink: 0 }}>
        {onBack && <BackButton onClick={onBack} />}
        <div>
          <div style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 800, fontSize: 22, color: TEXT, letterSpacing: '-0.2px' }}>Here's your plan</div>
          <div style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 500, fontSize: 13, color: TEXT_SUB }}>Adjust to fit your life</div>
        </div>
      </div>

      {/* Weekly schedule */}
      <div style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 700, fontSize: 13, color: TEXT_SUB, letterSpacing: 0.6, textTransform: 'uppercase', marginBottom: 10 }}>
        Weekly schedule
      </div>
        ...
        {visibleSessions.map(({ day, name, duration, focus }) => (
          <div key={day} style={{
            ...
            <div style={{
              ...
              fontFamily: 'Plus Jakarta Sans', fontWeight: 700, fontSize: 12, color: TEAL,
            }}>
              {day}
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 700, fontSize: 14, color: TEXT, marginBottom: 2 }}>{name}</div>
              <div style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 500, fontSize: 12, color: TEXT_SUB }}>{focus}</div>
            </div>
            <div style={{
          ...
          return (
            <button
              ...
              style={{
                ...
                boxShadow: chosen ? '0 2px 10px rgba(61,171,142,0.14)' : '0 1px 4px rgba(0,0,0,0.04)',
                transition: 'all 0.15s',
              }}
            >
              <div style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 700, fontSize: 13, color: chosen ? TEAL : TEXT, marginBottom: 3 }}>{label}</div>
              <div style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 500, fontSize: 10, color: TEXT_SUB, lineHeight: 1.3 }}>{desc}</div>
            </button>
          );
        })}
      </div>
      ...
      <div style={{ marginBottom: 26 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <div style={{ fontFamily: 'Plus Jakarta Sans', fontWeight: 700, fontSize: 13, color: TEXT_SUB, letterSpacing: 0.6, textTransform: 'uppercase' }}>
            Sessions / week
          </div>
          <div style={{
            fontFamily: 'Plus Jakarta Sans', fontWeight: 700, fontSize: 16, color: TEAL,
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
          <span style={{ fontFamily: 'Plus Jakarta Sans', fontSize: 11, color: TEXT_SUB }}>2 / week</span>
          <span style={{ fontFamily: 'Plus Jakarta Sans', fontSize: 11, color: TEXT_SUB }}>7 / week</span>
        </div>
      </div>
      ...
      <button
        onClick={onSave}
        style={{
          width: '100%', height: 52, borderRadius: 26, flexShrink: 0,
